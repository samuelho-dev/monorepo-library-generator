...
import { createWorkspaceContextExplicit } from "../workspace"
...
describe("Workspace Context", () => {
  describe("createWorkspaceContextExplicit", () => {
    ...
    it("should support all interface types", () => {
      const mcpContext = createWorkspaceContextExplicit("/test", "nx", "@scope", "pnpm", "mcp")
      expect(mcpContext.interfaceType).toBe("mcp")

      const cliContext = createWorkspaceContextExplicit("/test", "nx", "@scope", "pnpm", "cli")
      expect(cliContext.interfaceType).toBe("cli")

      const nxContext = createWorkspaceContextExplicit("/test", "nx", "@scope", "pnpm", "nx")
      expect(nxContext.interfaceType).toBe("nx")
    })

    it("should support all workspace types", () => {
      const nxContext = createWorkspaceContextExplicit("/test", "nx", "@scope", "pnpm", "mcp")
    ...
    it("should support all package managers", () => {
      const pnpmContext = createWorkspaceContextExplicit("/test", "nx", "@scope", "pnpm", "mcp")
      expect(pnpmContext.packageManager).toBe("pnpm")

      const npmContext = createWorkspaceContextExplicit("/test", "nx", "@scope", "npm", "mcp")
      expect(npmContext.packageManager).toBe("npm")

      const yarnContext = createWorkspaceContextExplicit("/test", "nx", "@scope", "yarn", "mcp")
      expect(yarnContext.packageManager).toBe("yarn")
    })
  })

  describe("createWorkspaceContext", () => {
    it("should use process.cwd() when rootPath is undefined", () => {
