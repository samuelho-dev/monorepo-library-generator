 ...
 * Generators may require TemplateCompiler for code generation.
 */
export type CoreGeneratorFn<TOptions> = (
  adapter: FileSystemAdapter,
  options: TOptions
) => Effect.Effect<GeneratorResult, FileSystemErrors | Error, TemplateCompiler>
  ...
  return {
    execute: (validated: ExtendedInput<TInput>) =>
      Effect.gen(function* () {
        ...
        yield* taggedGeneratorDuration(libraryType).pipe(Metric.update(duration))
