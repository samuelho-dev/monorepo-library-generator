  ...
  describe("TemplateCompiler", () => {
    ...
    it("should compile a simple template", async () => {
      const template: TemplateDefinition = {
        ...
        imports: [{ from: "effect", items: ["Effect", "Context"] }],
        sections: [
          {
            title: "Types",
            content: {
                ...
                staticLayers: [
                  {
                    name: "Live",
                    implementation:
                      "Layer.succeed({className}Repository, { findById: () => Effect.succeed({} as {className}) })"
                  }
                ]
              }
            }
    ...
    it("should handle conditional imports", async () => {
      ...
      // With condition
      const withRpc = await Effect.runPromise(compile(template, { ...context, includeRpc: true }))
    ...
    it("should handle conditional sections", async () => {
      ...
      // With condition
      const withCQRS = await Effect.runPromise(compile(template, { ...context, includeCQRS: true }))
    ...
    it("should handle conditionals block", async () => {
      const template: TemplateDefinition = {
        ...
        conditionals: {
          includeEvents: {
            imports: [{ from: "./events", items: ["{className}Event"] }],
            sections: [
              {
      ...
      // With condition
      const with_ = await Effect.runPromise(compile(template, { ...context, includeEvents: true }))
