 ...
 */
function buildSchemaExpression(config: SchemaFragmentConfig, context: TemplateContext) {
  let expr: string
...
function buildStructSchema(fields: ReadonlyArray<SchemaField>, context: TemplateContext) {
  ...
  const fieldLines = fields.map((field) => {
    ...
    const schemaExpr = field.optional ? `Schema.optional(${fieldSchema})` : fieldSchema
