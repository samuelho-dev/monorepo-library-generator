...
const BASE_CONTEXT = ["className", "fileName", "propertyName", "constantName", "scope", "packageName", "projectName", "libraryType"] as const
 ...
 */
export function getAvailableFileTypes(libraryType: LibraryType): ReadonlyArray<FileType> {
 ...
 */
export function getTemplate(libraryType: LibraryType, fileType: FileType): TemplateRegistryEntry | undefined {
  const registry = getTemplateRegistry()
  return registry.get(`${libraryType}/${fileType}` as TemplateKey)
}

/**
 * Check if a template exists for the given library and file type
 */
export function hasTemplate(libraryType: LibraryType, fileType: FileType): boolean {
  const registry = getTemplateRegistry()
  return registry.has(`${libraryType}/${fileType}` as TemplateKey)
}

/**
 * Get all registered library types
 */
export function getRegisteredLibraryTypes(): ReadonlyArray<LibraryType> {
  const registry = getTemplateRegistry()
  const types = new Set<LibraryType>()
  for (const key of registry.keys()) {
    const [libraryType] = key.split("/") as [LibraryType]
