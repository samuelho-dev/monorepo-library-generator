 ...
 */
function buildTags(libraryType: LibraryType, fileName: string, additionalTags?: string[]) {
 ...
 * @returns Effect that resolves to complete library metadata
 *
 * @example
 * ```typescript
 * // In generator wrapper (direct call, no intermediary)
 * const metadata = yield* computeLibraryMetadata(
 *   tree,
 *   schema,
 *   "contract",
 *   ["platform:universal"]
 ...
 * ```
 */
export const computeLibraryMetadata = (
  tree: Tree,
  schema: {
    readonly name: string
    readonly directory?: string
