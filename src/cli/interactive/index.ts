...
function executeGeneration(result: WizardResult) {
  return Effect.gen(function* () {
    ...
    const { generateContract } = yield* Effect.promise(() => import('../generators/contract'));
    const { generateDataAccess } = yield* Effect.promise(() => import('../generators/data-access'));
    const { generateFeature } = yield* Effect.promise(() => import('../generators/feature'));
    const { generateInfra } = yield* Effect.promise(() => import('../generators/infra'));
    const { generateProvider } = yield* Effect.promise(() => import('../generators/provider'));
    const { generateDomain } = yield* Effect.promise(() => import('../generators/domain'));
    ...
    switch (result.libraryType) {
      ...
      case 'domain':
        yield* generateDomain({
          ...
          ...(result.options.includeCache !== undefined && {
          ...
          ...(result.options.includeRPC !== undefined && { includeRPC: result.options.includeRPC }),
          ...(result.options.includeEdge !== undefined && {
            includeEdge: result.options.includeEdge,
          }),
        });
        break;
