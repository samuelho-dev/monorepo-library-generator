...
/**
 * Initialize wizard state with workspace context
 */
...
export function runWizard(
  ...
  return Effect.gen(function* () {
    const state = createInitialState(workspaceRoot, librariesRoot);
    ...
    if (typeOptions.length > 0) {
      for (const opt of typeOptions) {
        ...
        (options as Record<string, boolean>)[opt.key] = value;
      }
      yield* Console.log("");
    } else {
