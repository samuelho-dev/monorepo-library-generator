...
describe('Contract Generator - Foundation', () => {
  ...
  describe('Base File Generation', () => {
    it('should generate all base files', async () => {
      ...
      // Entity types directory (for prisma-effect-kysely integration)
      expect(tree.exists('libs/contract/product/src/lib/types/database.ts')).toBe(true);
...
describe('Contract Generator - Exports', () => {
  ...
  describe('Conditional Exports in index.ts', () => {
    it('should export core domain types in all cases', async () => {
      ...
      // Verify database types file exists with entity types
      // This file defines branded IDs and entity schemas for prisma-effect-kysely integration
      const typesContent = tree.read('libs/contract/product/src/lib/types/database.ts', 'utf-8');
