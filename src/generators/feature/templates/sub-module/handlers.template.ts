...
- Middleware (auth, service-auth, request-meta) is applied automatically
  ...
  builder.addImports([
    { from: `${scope}/contract-${parentName}`, imports: [subModuleClassName] },
    {
      ...
      imports: [`${subModuleClassName}NotFoundRpcError`, `${subModuleClassName}ValidationRpcError`]
    }
  ])

  // Import types for error mapping (RpcError union type and domain error)
  builder.addImports([
    {
      from: `${scope}/contract-${parentName}/${subModuleName}`,
      imports: [`${subModuleClassName}RpcError`, `${subModuleClassName}Error`],
      isTypeOnly: true
    }
  ])
