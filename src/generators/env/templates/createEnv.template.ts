...
export function generateCreateEnvFile() {
  ...
  // Imports - Node.js default imports (not supported by addImport)
  builder.addRaw('import fs from "node:fs"')
  builder.addRaw('import path from "node:path"')
  ...
  builder.addSectionComment('Load .env File')
  builder.addRaw('/**')
  builder.addRaw(' * Load .env file synchronously and merge with process.env')
  builder.addRaw(' *')
  builder.addRaw(' * Returns a Map of all environment variables with .env values')
  builder.addRaw(' * overriding process.env where both exist.')
  builder.addRaw(' */')
  builder.addRaw('function loadEnvSync(): Map<string, string> {')
  builder.addRaw('  const envMap = new Map<string, string>()')
  builder.addRaw('  // First, load from process.env')
  builder.addRaw('  // biome-ignore lint/style/noProcessEnv: This is the env library - it wraps process.env for all other code')
  builder.addRaw('  for (const [key, value] of Object.entries(process.env)) {')
  builder.addRaw('    if (value !== undefined) {')
  builder.addRaw('      envMap.set(key, value)')
  builder.addRaw('    }')
  builder.addRaw('  }')
  builder.addRaw('  // On server, also read .env file if it exists')
  builder.addRaw('  if (isServer) {')
  builder.addRaw('    try {')
  builder.addRaw('      const envPath = path.resolve(process.cwd(), ".env")')
  builder.addRaw('      const content = fs.readFileSync(envPath, "utf-8")')
  builder.addRaw('      const dotEnvVars = parseDotEnv(content)')
  builder.addRaw('      // .env values override process.env')
  builder.addRaw('      for (const [key, value] of Object.entries(dotEnvVars)) {')
  builder.addRaw('        envMap.set(key, value)')
  builder.addRaw('      }')
  ...
  builder.addRaw('/**')
  builder.addRaw(' * ConfigProvider from merged process.env + .env file')
  builder.addRaw(' */')
  builder.addRaw('const ConfigLayer = Layer.succeed(')
  builder.addRaw('  ConfigProvider.ConfigProvider,')
  builder.addRaw('  ConfigProvider.fromMap(loadEnvSync())')
  builder.addRaw(')')
  builder.addBlankLine()
  ...
  builder.addRaw(') {')
  builder.addRaw('  const { server, client, shared, clientPrefix } = options')
