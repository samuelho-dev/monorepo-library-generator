/**
 * PubSub Service Interface Template
 *
 * Generates proper Effect.PubSub wrapper with publish/subscribe patterns.
 * Supports Redis backing for distributed messaging.
 *
...
export function generatePubSubInterfaceFile(options: InfraTemplateOptions) {
  ...
  builder.addFileHeader({
    ...
    description: `PubSub infrastructure using Effect.PubSub primitive.
  ...
  static readonly Dev = Layer.scoped(
    ...
    Effect.gen(function*() {
      ...
      const makeTopicHandle = <A, E>(
        ...
        publish: (message: A) =>
          Effect.gen(function*() {
            yield* Effect.logDebug("[${className}Service] [DEV] publish", {
              topic: topicName,
            ...
            const validated = yield* validate(message).pipe(
              Effect.tapError((error) =>
                Effect.gen(function*() {
                  ...
                  yield* Effect.logError("[${className}Service] [DEV] Validation error", {
                    topic: topicName,
                    error: String(error),
            ...
            yield* Effect.logDebug("[${className}Service] [DEV] Message published", {
              topic: topicName,
              published
            })
        ...
        publishAll: (messages: Iterable<A>) =>
          Effect.gen(function*() {
            ...
            yield* Effect.logDebug("[${className}Service] [DEV] publishAll", {
              topic: topicName,
            ...
            yield* Effect.logDebug("[${className}Service] [DEV] Batch published", {
              topic: topicName,
              count: messageArray.length,
              published
            })
        ...
        subscribe: Effect.gen(function*() {
          ...
          yield* Effect.logInfo("[${className}Service] [DEV] Subscribed to topic", {
            topic: topicName
          })
 ...
 * \`\`\`typescript
 * import * as Observability from "${scope}/infra-observability";
 *
