/**
 * Logging Service Template (Part of Observability)
 *
 ...
 *
 * This template generates a logging service that integrates with the
 * observability infrastructure. When the OTEL SDK layer is provided,
 * logs are automatically exported via OpenTelemetry.
 */
export function generateLoggingServiceFile(options: InfraTemplateOptions) {
  const builder = new TypeScriptBuilder()
  const { className, fileName } = options
  const scope = WORKSPACE_CONFIG.getScope()

  builder.addFileHeader({
    title: `${className} Logging Service`,
 ...
 * @example
 * \`\`\`typescript
 * import { LoggingService } from "${scope}/infra-${fileName}";
 *
...
export class LoggingService extends Context.Tag(
  "${scope}/infra-${fileName}/LoggingService"
)<
  LoggingService,
  LoggingOperations
>() {
