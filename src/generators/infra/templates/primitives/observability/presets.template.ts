/**
 * Observability Presets Template
 *
 * Generates pre-configured OTEL SDK layers for common deployment scenarios.
 ...
 */
export function generateObservabilityPresetsFile(options: InfraTemplateOptions) {
  const builder = new TypeScriptBuilder()
  const { className, fileName } = options
  const scope = WORKSPACE_CONFIG.getScope()

  builder.addFileHeader({
    title: `${className} Presets`,
    description: `Pre-configured OTEL SDK layers for common deployment scenarios.
...
Each preset is ready to use with minimal configuration.`,
    module: `${scope}/infra-${fileName}/presets`
  })
  ...
  builder.addImports([
    ...
    { from: "@opentelemetry/sdk-metrics", imports: ["PeriodicExportingMetricReader", "ConsoleMetricExporter"] }
  ])
