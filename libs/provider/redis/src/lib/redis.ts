...
import { makeCacheClient } from './cache'
 ...
 * Redis Service
 *
 * Main Redis provider service with Effect integration.
 ...
 *
 * @module @samuelho-dev/provider-redis/service
...
export interface RedisServiceInterface {
  ...
  readonly scan: (
    cursor: number,
    options?: ScanOptions
  ) => Effect.Effect<ScanResult, RedisCommandError>
 ...
 */
export class RedisService extends Context.Tag('Redis')<RedisService, RedisServiceInterface>() {
  /**
 ...
 */
const RedisScanResponse = Schema.Tuple(Schema.String, Schema.Array(Schema.String))
 ...
 */
function makeRedisService(config: RedisConfig, mainClient: IORedis, subClient: IORedis) {
  return {
    config,
