...
export const env = createEnv({
  ...
  server: {
    ...
    // Supabase
    SUPABASE_URL: Config.string('SUPABASE_URL').pipe(Config.withDefault('http://localhost:54321')),
    ...
    JWT_SECRET: Config.string('JWT_SECRET').pipe(Config.withDefault('dev-jwt-secret')),

    // OpenTelemetry
    OTEL_SERVICE_NAME: Config.string('OTEL_SERVICE_NAME').pipe(Config.withDefault('dev-service')),
    OTEL_SERVICE_VERSION: Config.string('OTEL_SERVICE_VERSION').pipe(Config.withDefault('0.0.0')),
    ...
    OTEL_TRACES_ENABLED: Config.string('OTEL_TRACES_ENABLED').pipe(Config.withDefault('true')),
    OTEL_METRICS_ENABLED: Config.string('OTEL_METRICS_ENABLED').pipe(Config.withDefault('true')),
  ...
  client: {
    PUBLIC_API_URL: Config.string('PUBLIC_API_URL').pipe(Config.withDefault('/api/rpc'))
  },
