...
- **lib/client/hooks/**: React hooks
  - `use-pubsub.ts`: Client-side hook (~2 KB)

### Import Patterns (Most to Least Optimized)

```typescript
// 1. Type-only import (zero runtime ~0.3 KB)
import type { PubsubConfig } from '@myorg/infra-pubsub/types';

// 2. Specific provider (smallest bundle ~4 KB)
import { MemoryProvider } from '@myorg/infra-pubsub/providers/memory';

// 3. Service interface (~5 KB)
import { PubsubService } from '@myorg/infra-pubsub/service';

// 4. Specific layer (~3-4 KB)
import { PubsubServiceLive } from '@myorg/infra-pubsub/layers/server-layers';

// 5. Client hook (~2 KB)
import { usePubsub } from '@myorg/infra-pubsub/client/hooks';

// 6. Platform barrel (~10-15 KB)
import { PubsubService } from '@myorg/infra-pubsub/server';

// 7. Package barrel (largest ~20 KB)
import { PubsubService } from '@myorg/infra-pubsub';
```
...
import { PubsubServiceLive } from '@myorg/infra-pubsub';

Effect.gen(function* () {
  const service = yield* PubsubService;
  // ...
}).pipe(
  Effect.provide(PubsubServiceLive)
);
```
