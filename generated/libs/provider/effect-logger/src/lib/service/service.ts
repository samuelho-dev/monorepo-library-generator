 ...
 *
 * Context.Tag definition for EffectLogger provider service.

External Service: Effect.Logger
...
Bundle optimization:
  ...
  - Full service: import { EffectLogger } from './service'
 *
 ...
 * EffectLogger Service Interface
 *
 * Provider: External service adapter for Effect.Logger
 *
 * Operations:
 ...
 */
export interface EffectLoggerServiceInterface {
  /**
   * Service configuration (read-only)
   */
  readonly config: EffectLoggerConfig;
  ...
  // ) => Stream.Stream<Resource, EffectLoggerServiceError, never>;
  //
  // // Implementation:
  // streamAll: (params) =>
  //   Stream.asyncScoped<Resource, EffectLoggerServiceError>((emit) =>
  //     Effect.gen(function* () {
  //       const client = yield* EffectLoggerClient;
  //       let page = 1;
  //       let hasMore = true;
  //
  //       while (hasMore) {
  ...
  // streamEvents: () =>
  //   Stream.asyncScoped<EffectLoggerEvent, EffectLoggerServiceError>((emit) =>
  //     Effect.gen(function* () {
  //       const client = yield* EffectLoggerClient;
  //
  ...
  // readonly bulkDelete: (
  //   ids: readonly string[]
  // ) => Stream.Stream<void, EffectLoggerServiceError, never>;
  //
  // // Implementation:
  // bulkDelete: (ids) =>
 ...
 *
 * Access via: yield* EffectLogger
 *
 * Static layers:
 * - EffectLogger.Live - Production with real Effect.Logger SDK
 * - EffectLogger.Test - In-memory for testing
 * - EffectLogger.Dev - Development with logging
 *
 * Bundle optimization:
 ...
 */
export class EffectLogger extends Context.Tag("EffectLogger")<
  EffectLogger,
  EffectLoggerServiceInterface
>() {
  /**
   * Live Layer - Production implementation
   *
   * Currently uses in-memory baseline. Replace with Effect.Logger SDK integration:
   *
   * @example
   * ```typescript
   * // 1. Install SDK: pnpm add effect.logger-sdk
   * // 2. Replace in-memory store with SDK calls:
   * static readonly Live = Layer.effect(
   *   this,
   *   Effect.gen(function* () {
   *     const client = new Effect.LoggerClient(config);
   *     return {
   *       get: (id) => Effect.tryPromise({
   *         try: () => client.get(id),
   *         catch: (error) => new EffectLoggerInternalError({ message: "Get failed", cause: error })
   *       }),
   *       // ... other methods
   *     };
   *   })
  ...
  static readonly Live = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      // TODO: Replace with Effect.Logger SDK integration
      const store = new Map<string, Resource>();
      let idCounter = 0;

      // Configuration from environment variables
      const config: EffectLoggerConfig = {
        apiKey: Redacted.value(env.EFFECT_LOGGER_API_KEY) ?? "fallback_api_key",
  ...
  static readonly Dev = Layer.effect(
    EffectLogger,
    Effect.gen(function* () {
      console.log("[EffectLogger] [DEV] Initializing development layer");
      ...
      return {
        ...
        healthCheck: Effect.gen(function* () {
          console.log("[EffectLogger] [DEV] healthCheck called");
          const result = yield* liveService.healthCheck;
          console.log("[EffectLogger] [DEV] healthCheck result:", result);
          return result;
        }),
        ...
        get: (id) =>
          Effect.gen(function* () {
            console.log("[EffectLogger] [DEV] get called with id:", id);
            const result = yield* liveService.get(id);
            console.log("[EffectLogger] [DEV] get result:", result);
            return result;
          }),

        create: (data) =>
          Effect.gen(function* () {
            console.log("[EffectLogger] [DEV] create called with:", data);
            const result = yield* liveService.create(data);
            console.log("[EffectLogger] [DEV] create result:", result);
            return result;
          }),

        update: (id, data) =>
          Effect.gen(function* () {
            console.log("[EffectLogger] [DEV] update called with id:", id, "data:", data);
            const result = yield* liveService.update(id, data);
            console.log("[EffectLogger] [DEV] update result:", result);
            return result;
          }),
