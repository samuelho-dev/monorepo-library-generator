# @myorg/provider-effect-logger

Effect.Logger provider for logging operations

## Quick Reference

This is an AI-optimized reference for Effect.Logger, a provider library following Effect-based service patterns with granular bundle optimization.
...
import { createOperations } from '@myorg/provider-effect-logger/service/operations/create';

// 2. Type-only import (zero runtime ~0.3 KB)
import type { Resource, EffectLoggerConfig } from '@myorg/provider-effect-logger/types';

// 3. Operation category (~8-10 KB)
import { createOperations, queryOperations } from '@myorg/provider-effect-logger/service/operations';

// 4. Full service (~12-15 KB)
import { EffectLogger } from '@myorg/provider-effect-logger/service';
...
1. **Configure External Service** (`lib/service/interface.ts`):
   - Initialize Effect.Logger SDK client in Live layer
...
import { createOperations } from '@myorg/provider-effect-logger/service/operations/create';
import type { Resource } from '@myorg/provider-effect-logger/types';
...
Follow these steps to integrate with the actual Effect.Logger SDK:

#### 1. Install SDK Package

```bash
pnpm add effect.logger-sdk
# Or the actual package name for Effect.Logger
```
...
export const EffectLoggerLive = Layer.effect(
  ...
  Effect.gen(function* () {
    const config: EffectLoggerConfig = {
      apiKey: env.EFFECT_LOGGER_API_KEY,
      timeout: env.EFFECT_LOGGER_TIMEOUT || 20000,
    };

    // Initialize SDK client
    const client = new Effect.LoggerSDK(config);

    return {
      config,
      healthCheck: Effect.succeed({ status: "healthy" as const })
        .pipe(Effect.withSpan("EffectLogger.healthCheck")),
      ...
      list: (params) =>
        Effect.tryPromise({
          ...
          catch: (error) => new EffectLoggerInternalError({
          ...
          Effect.timeoutFail({
            ...
            onTimeout: () => new EffectLoggerTimeoutError({
              message: "list operation timed out",
              timeoutMs: config.timeout,
              operation: "list"
            })
          }),
          Effect.withSpan("EffectLogger.list")
        ),
...
export const EffectLoggerLive = Layer.scoped(
  ...
  Effect.gen(function* () {
    ...
    const client = yield* Effect.acquireRelease(
      Effect.tryPromise(() => Effect.LoggerSDK.connect(config)),
...
list: (params) =>
  Effect.tryPromise({
    ...
    Effect.tap(() => Effect.logDebug("[EffectLogger] list called", params)),
    Effect.timeoutFail({ /* ... */ }),
    Effect.withSpan("EffectLogger.list")
  ),
```
...
Replace in-memory store in `makeEffectLoggerLayer`:
