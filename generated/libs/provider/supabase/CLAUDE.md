# @myorg/provider-supabase

Supabase provider for auth, storage, and client operations

## Quick Reference

This is an AI-optimized reference for Supabase, a provider library following Effect-based service patterns with granular bundle optimization.
...
import { createOperations } from '@myorg/provider-supabase/service/operations/create';

// 2. Type-only import (zero runtime ~0.3 KB)
import type { Resource, SupabaseConfig } from '@myorg/provider-supabase/types';

// 3. Operation category (~8-10 KB)
import { createOperations, queryOperations } from '@myorg/provider-supabase/service/operations';

// 4. Full service (~12-15 KB)
import { Supabase } from '@myorg/provider-supabase/service';

// 5. Package barrel (largest ~18-20 KB)
import { Supabase } from '@myorg/provider-supabase';
```
...
import { createOperations } from '@myorg/provider-supabase/service/operations/create';
import type { Resource } from '@myorg/provider-supabase/types';
...
```typescript
export const SupabaseLive = Layer.scoped(
  Supabase,
  Effect.gen(function* () {
    const config: SupabaseConfig = {
      apiKey: env.SUPABASE_API_KEY,
...
Replace in-memory store in `makeSupabaseLayer`:

```typescript
export function makeSupabaseLayer(config: SupabaseConfig) {
  return Layer.scoped(
    Supabase,
    Effect.gen(function* () {
      const client = new SupabaseSDK(config);
