...
//     );
//
//     return EffectPubsub.make(client, config);
//   })
// );
//
// // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // //
...
//   Effect.gen(function* () {
//     const config: EffectPubsubConfig = {
//       apiKey: env.EFFECT_PUBSUB_API_KEY ?? "baseline_api_key",
//       timeout: env.EFFECT_PUBSUB_TIMEOUT ?? 20000,
//     };
//
...
//         // client.disconnect()
//         // pool.end()
//         console.log(`[EffectPubsub] Cleaning up client resources`);
//       }),
//     );
//
 ...
 */
export const EffectPubsubDev = Layer.effect(
  EffectPubsub,
  Effect.gen(function* () {
    const config: EffectPubsubConfig = {
      apiKey: Redacted.value(env.EFFECT_PUBSUB_API_KEY) ?? "dev_key",
 ...
 */
export const EffectPubsubAuto = Layer.suspend(() => {
  switch (env.NODE_ENV) {
    case "production":
      return EffectPubsubLive;
    case "development":
      return EffectPubsubDev;
    case "test":
      return EffectPubsubTest;
    default:
      return EffectPubsubDev;
  }
});
...
/**
 * makeEffectPubsubLayer - Custom layer factory
 *
 ...
 * Example:
 * ```typescript
 * const customLayer = makeEffectPubsubLayer({
 *   apiKey: "custom_key",
 *   timeout: 5000,
 * });
 * ```
...
export function makeEffectPubsubLayer(config: EffectPubsubConfig) {
  return Layer.scoped(
    ...
    Effect.gen(function* () {
      ...
      // Register cleanup function
      yield* Effect.addFinalizer(() =>
        Effect.logInfo("[EffectPubsub] [CUSTOM] Cleaning up resources").pipe(
          Effect.andThen(
            Effect.sync(() => {
