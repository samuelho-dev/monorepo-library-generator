# @myorg/provider-kysely

Kysely provider for type-safe database queries with migrations

## Quick Reference

This is an AI-optimized reference for Kysely, a provider library following Effect-based service patterns with granular bundle optimization.
...
import { createOperations } from '@myorg/provider-kysely/service/operations/create';

// 2. Type-only import (zero runtime ~0.3 KB)
import type { Resource, KyselyConfig } from '@myorg/provider-kysely/types';

// 3. Operation category (~8-10 KB)
import { createOperations, queryOperations } from '@myorg/provider-kysely/service/operations';

// 4. Full service (~12-15 KB)
import { Kysely } from '@myorg/provider-kysely/service';
...
1. **Configure External Service** (`lib/service/interface.ts`):
   - Initialize Kysely SDK client in Live layer
...
import { createOperations } from '@myorg/provider-kysely/service/operations/create';
import type { Resource } from '@myorg/provider-kysely/types';
...
Follow these steps to integrate with the actual Kysely SDK:

#### 1. Install SDK Package

```bash
pnpm add kysely-sdk
# Or the actual package name for Kysely
```
...
export const KyselyLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const client = new KyselySDK(config);

    return {
      config,
      healthCheck: Effect.succeed({ status: "healthy" as const })
        .pipe(Effect.withSpan("Kysely.healthCheck")),
      ...
      list: (params) =>
        Effect.tryPromise({
          ...
          catch: (error) => new KyselyInternalError({
          ...
          Effect.timeoutFail({
            ...
            onTimeout: () => new KyselyTimeoutError({
              message: "list operation timed out",
              timeoutMs: config.timeout,
              operation: "list"
            })
          }),
          Effect.withSpan("Kysely.list")
        ),
...
```typescript
export const KyselyLive = Layer.scoped(
  Kysely,
  Effect.gen(function* () {
    const config: KyselyConfig = {
      apiKey: env.KYSELY_API_KEY,
    ...
    const client = yield* Effect.acquireRelease(
      Effect.tryPromise(() => KyselySDK.connect(config)),
...
list: (params) =>
  Effect.tryPromise({
    ...
    Effect.tap(() => Effect.logDebug("[Kysely] list called", params)),
    Effect.timeoutFail({ /* ... */ }),
    Effect.withSpan("Kysely.list")
  ),
```
...
Replace in-memory store in `makeKyselyLayer`:

```typescript
export function makeKyselyLayer(config: KyselyConfig) {
  return Layer.scoped(
    Kysely,
    Effect.gen(function* () {
      const client = new KyselySDK(config);
