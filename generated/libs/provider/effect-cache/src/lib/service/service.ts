 ...
 *
 * Context.Tag definition for EffectCache provider service.

External Service: Effect.Cache
...
Bundle optimization:
  ...
  - Full service: import { EffectCache } from './service'
 *
 ...
 * EffectCache Service Interface
 *
 * Provider: External service adapter for Effect.Cache
 *
 * Operations:
 ...
 */
export interface EffectCacheServiceInterface {
  /**
   * Service configuration (read-only)
   */
  readonly config: EffectCacheConfig;
  ...
  // ) => Stream.Stream<Resource, EffectCacheServiceError, never>;
  //
  // // Implementation:
  // streamAll: (params) =>
  //   Stream.asyncScoped<Resource, EffectCacheServiceError>((emit) =>
  //     Effect.gen(function* () {
  //       const client = yield* EffectCacheClient;
  //       let page = 1;
  //       let hasMore = true;
  //
  //       while (hasMore) {
  ...
  // streamEvents: () =>
  //   Stream.asyncScoped<EffectCacheEvent, EffectCacheServiceError>((emit) =>
  //     Effect.gen(function* () {
  //       const client = yield* EffectCacheClient;
  //
  ...
  // readonly bulkDelete: (
  //   ids: readonly string[]
  // ) => Stream.Stream<void, EffectCacheServiceError, never>;
  //
  // // Implementation:
  // bulkDelete: (ids) =>
 ...
 *
 * Access via: yield* EffectCache
 *
 * Static layers:
 * - EffectCache.Live - Production with real Effect.Cache SDK
 * - EffectCache.Test - In-memory for testing
 * - EffectCache.Dev - Development with logging
 *
 * Bundle optimization:
 ...
 */
export class EffectCache extends Context.Tag("EffectCache")<
  EffectCache,
  EffectCacheServiceInterface
>() {
  /**
   * Live Layer - Production implementation
   *
   * Currently uses in-memory baseline. Replace with Effect.Cache SDK integration:
   *
   * @example
   * ```typescript
   * // 1. Install SDK: pnpm add effect.cache-sdk
   * // 2. Replace in-memory store with SDK calls:
   * static readonly Live = Layer.effect(
   *   this,
   *   Effect.gen(function* () {
   *     const client = new Effect.CacheClient(config);
   *     return {
   *       get: (id) => Effect.tryPromise({
   *         try: () => client.get(id),
   *         catch: (error) => new EffectCacheInternalError({ message: "Get failed", cause: error })
   *       }),
   *       // ... other methods
   *     };
   *   })
  ...
  static readonly Live = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      // TODO: Replace with Effect.Cache SDK integration
      const store = new Map<string, Resource>();
      let idCounter = 0;

      // Configuration from environment variables
      const config: EffectCacheConfig = {
        apiKey: Redacted.value(env.EFFECT_CACHE_API_KEY) ?? "fallback_api_key",
  ...
  static readonly Dev = Layer.effect(
    EffectCache,
    Effect.gen(function* () {
      console.log("[EffectCache] [DEV] Initializing development layer");
      ...
      return {
        ...
        healthCheck: Effect.gen(function* () {
          console.log("[EffectCache] [DEV] healthCheck called");
          const result = yield* liveService.healthCheck;
          console.log("[EffectCache] [DEV] healthCheck result:", result);
          return result;
        }),
        ...
        get: (id) =>
          Effect.gen(function* () {
            console.log("[EffectCache] [DEV] get called with id:", id);
            const result = yield* liveService.get(id);
            console.log("[EffectCache] [DEV] get result:", result);
            return result;
          }),

        create: (data) =>
          Effect.gen(function* () {
            console.log("[EffectCache] [DEV] create called with:", data);
            const result = yield* liveService.create(data);
            console.log("[EffectCache] [DEV] create result:", result);
            return result;
          }),

        update: (id, data) =>
          Effect.gen(function* () {
            console.log("[EffectCache] [DEV] update called with id:", id, "data:", data);
            const result = yield* liveService.update(id, data);
            console.log("[EffectCache] [DEV] update result:", result);
            return result;
          }),
