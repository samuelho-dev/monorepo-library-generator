import { EffectCacheInternalError } from "./errors"
import { EffectCache } from "./service"
import { env } from "@myorg/env"
import { Effect, Layer, Redacted } from "effect"
import type { EffectCacheConfig, Resource } from "./types"

/**
 * effect-cache - Layer Implementations
 *
  ...
  return {
    ...
    get: (id: string) =>
      Effect.gen(function* () {
        ...
        if (!item) {
          return yield* Effect.fail(new EffectCacheInternalError({ message: `Item ${id} not found` }));
        }
        return item;
      }),
...
// Example pattern:
//
// export const EffectCacheLive = Layer.scoped(
//   EffectCache,
//   Effect.gen(function* () {
...
//     );
//
//     return EffectCache.make(client, config);
//   })
// );
//
// // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // 
 ...
 */
export const EffectCacheLive = Layer.effect(
  EffectCache,
  Effect.sync(() => {
    const config: EffectCacheConfig = {
      apiKey: Redacted.value(env.EFFECT_CACHE_API_KEY) ?? "baseline_api_key",
      timeout: env.EFFECT_CACHE_TIMEOUT ?? 20000
    }
    ...
    return {
      config,
      healthCheck: Effect.succeed({ status: "healthy" as const }).pipe(Effect.withSpan("EffectCache.healthCheck")),
      // TODO: Replace store operations with SDK calls
      list: (params) => store.list(params).pipe(Effect.withSpan("EffectCache.list")),
      get: (id) => store.get(id).pipe(Effect.withSpan("EffectCache.get")),
      create: (data) => store.create(data).pipe(Effect.withSpan("EffectCache.create")),
      update: (id, data) => store.update(id, data).pipe(Effect.withSpan("EffectCache.update")),
      delete: (id) => store.delete(id).pipe(Effect.withSpan("EffectCache.delete"))
    }
  })
)
...
//   Effect.gen(function* () {
//     const config: EffectCacheConfig = {
//       apiKey: env.EFFECT_CACHE_API_KEY ?? "baseline_api_key",
//       timeout: env.EFFECT_CACHE_TIMEOUT ?? 20000,
//     };
//
...
//         // client.disconnect()
//         // pool.end()
//         console.log(`[EffectCache] Cleaning up client resources`);
//       }),
//     );
//
 ...
 */
export const EffectCacheTest = Layer.sync(
  EffectCache,
  () => {
 ...
 */
export const EffectCacheDev = Layer.effect(
  EffectCache,
  Effect.gen(function* () {
    const config: EffectCacheConfig = {
      apiKey: Redacted.value(env.EFFECT_CACHE_API_KEY) ?? "dev_key",
    ...
    yield* Effect.logInfo("[EffectCache] [DEV] Development layer initialized with in-memory store");
    ...
    return {
      ...
      healthCheck: Effect.succeed({ status: "healthy" as const }).pipe(Effect.withSpan("EffectCache.healthCheck")),
      // TODO: Replace store operations with SDK calls
      list: (params) => store.list(params).pipe(Effect.tap(() => Effect.logDebug("[EffectCache] list called"))).pipe(Effect.withSpan("EffectCache.list")),
      get: (id) => store.get(id).pipe(Effect.tap(() => Effect.logDebug(`[EffectCache] get called: ${id}`))).pipe(Effect.withSpan("EffectCache.get")),
      create: (data) => store.create(data).pipe(Effect.tap(() => Effect.logDebug("[EffectCache] create called"))).pipe(Effect.withSpan("EffectCache.create")),
      update: (id, data) => store.update(id, data).pipe(Effect.tap(() => Effect.logDebug(`[EffectCache] update called: ${id}`))).pipe(Effect.withSpan("EffectCache.update")),
      delete: (id) => store.delete(id).pipe(Effect.tap(() => Effect.logDebug(`[EffectCache] delete called: ${id}`))).pipe(Effect.withSpan("EffectCache.delete")),
    };
  }),
);
 ...
 */
export const EffectCacheAuto = Layer.suspend(() => {
  switch (env.NODE_ENV) {
    case "production":
      return EffectCacheLive;
    case "development":
      return EffectCacheDev;
    case "test":
      return EffectCacheTest;
    default:
      return EffectCacheDev;
  }
});
...
/**
 * makeEffectCacheLayer - Custom layer factory
 *
 ...
 * Example:
 * ```typescript
 * const customLayer = makeEffectCacheLayer({
 *   apiKey: "custom_key",
 *   timeout: 5000,
 * });
 * ```
 */
export function makeEffectCacheLayer(config: EffectCacheConfig) {
  return Layer.scoped(
    EffectCache,
    Effect.gen(function* () {
      // Baseline: In-memory store (replace with SDK integration)
      ...
      // Register cleanup function
      yield* Effect.addFinalizer(() =>
        Effect.logInfo("[EffectCache] [CUSTOM] Cleaning up resources").pipe(
          Effect.andThen(
            Effect.sync(() => {
