      ...
      Effect.gen(function* () {
        ...
        if (!item) {
          return yield* Effect.fail(
            new EffectCacheInternalError({ message: `Item ${id} not found` }),
          );
        }
        return item;
      }),
  ...
  Effect.sync(() => {
    ...
    return {
      ...
      healthCheck: Effect.succeed({ status: "healthy" as const }).pipe(
        Effect.withSpan("EffectCache.healthCheck"),
      ),
 ...
 */
export const EffectCacheTest = Layer.sync(EffectCache, () => {
  ...
  Effect.gen(function* () {
    ...
    return {
      ...
      list: (params) =>
        store
          .list(params)
          .pipe(Effect.tap(() => Effect.logDebug("[EffectCache] list called")))
          .pipe(Effect.withSpan("EffectCache.list")),
      get: (id) =>
        store
          .get(id)
          .pipe(Effect.tap(() => Effect.logDebug(`[EffectCache] get called: ${id}`)))
          .pipe(Effect.withSpan("EffectCache.get")),
      create: (data) =>
        store
          .create(data)
          .pipe(Effect.tap(() => Effect.logDebug("[EffectCache] create called")))
          .pipe(Effect.withSpan("EffectCache.create")),
      update: (id, data) =>
        store
          .update(id, data)
          .pipe(Effect.tap(() => Effect.logDebug(`[EffectCache] update called: ${id}`)))
          .pipe(Effect.withSpan("EffectCache.update")),
      delete: (id) =>
        store
          .delete(id)
          .pipe(Effect.tap(() => Effect.logDebug(`[EffectCache] delete called: ${id}`)))
          .pipe(Effect.withSpan("EffectCache.delete")),
    };
  }),
);
