 ...
 *
 * @module @myorg/provider-effect-cache
 */
 ...
 */
class EffectCacheTestService extends Context.Tag("EffectCacheTestService")<
  EffectCacheTestService,
  {
 ...
 */
function createEffectCacheTestLayer(config: Record<string, unknown> = {}) {
  return Layer.succeed(EffectCacheTestService, {
    getName: () => Effect.succeed("effect-cache"),
    getConfig: () => Effect.succeed(config),
  });
}

describe("EffectCache Service", () => {
  describe("Service Interface", () => {
    it.scoped("should provide service through layer", () =>
      Effect.gen(function* () {
        const service = yield* EffectCacheTestService;
        const name = yield* service.getName();
        expect(name).toBe("effect-cache");
      }).pipe(Effect.provide(Layer.fresh(createEffectCacheTestLayer()))),
    );
    ...
    it.scoped("should provide configuration", () =>
      ...
      }).pipe(Effect.provide(Layer.fresh(createEffectCacheTestLayer({ timeout: 5000 })))),
    );
  });
  ...
  describe("Layer Composition", () => {
    ...
    it.scoped("should allow layer override", () => {
      const overrideLayer = Layer.succeed(EffectCacheTestService, {
  ...
  describe("Layer Types", () => {
    it.scoped("should work with Layer.succeed for synchronous initialization", () => {
      const syncLayer = Layer.succeed(EffectCacheTestService, {
        getName: () => Effect.succeed("sync-effect-cache"),
      ...
      return Effect.gen(function* () {
        const service = yield* EffectCacheTestService;
        const name = yield* service.getName();
        expect(name).toBe("sync-effect-cache");
    ...
    it.scoped("should work with Layer.effect for async initialization", () => {
      const asyncLayer = Layer.effect(
        EffectCacheTestService,
        Effect.sync(() => ({
          getName: () => Effect.succeed("async-effect-cache"),
      ...
      return Effect.gen(function* () {
        const service = yield* EffectCacheTestService;
        const name = yield* service.getName();
        expect(name).toBe("async-effect-cache");
    ...
    it.scoped("should work with Layer.scoped for resource management", () => {
      ...
      const scopedLayer = Layer.scoped(
        ...
        Effect.acquireRelease(
          Effect.sync(() => {
            initialized = true;
            return {
              getName: () => Effect.succeed("scoped-effect-cache"),
      ...
      return Effect.gen(function* () {
        const service = yield* EffectCacheTestService;
        const name = yield* service.getName();
        expect(name).toBe("scoped-effect-cache");
