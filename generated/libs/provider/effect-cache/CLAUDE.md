# @myorg/provider-effect-cache

Effect.Cache provider for caching operations

## Quick Reference

This is an AI-optimized reference for Effect.Cache, a provider library following Effect-based service patterns with granular bundle optimization.
...
import { createOperations } from '@myorg/provider-effect-cache/service/operations/create';

// 2. Type-only import (zero runtime ~0.3 KB)
import type { Resource, EffectCacheConfig } from '@myorg/provider-effect-cache/types';

// 3. Operation category (~8-10 KB)
import { createOperations, queryOperations } from '@myorg/provider-effect-cache/service/operations';

// 4. Full service (~12-15 KB)
import { EffectCache } from '@myorg/provider-effect-cache/service';
...
1. **Configure External Service** (`lib/service/interface.ts`):
   - Initialize Effect.Cache SDK client in Live layer
...
import { createOperations } from '@myorg/provider-effect-cache/service/operations/create';
import type { Resource } from '@myorg/provider-effect-cache/types';
...
Follow these steps to integrate with the actual Effect.Cache SDK:

#### 1. Install SDK Package

```bash
pnpm add effect.cache-sdk
# Or the actual package name for Effect.Cache
```
...
export const EffectCacheLive = Layer.effect(
  ...
  Effect.gen(function* () {
    const config: EffectCacheConfig = {
      apiKey: env.EFFECT_CACHE_API_KEY,
      timeout: env.EFFECT_CACHE_TIMEOUT || 20000,
    };

    // Initialize SDK client
    const client = new Effect.CacheSDK(config);

    return {
      config,
      healthCheck: Effect.succeed({ status: "healthy" as const })
        .pipe(Effect.withSpan("EffectCache.healthCheck")),
      ...
      list: (params) =>
        Effect.tryPromise({
          ...
          catch: (error) => new EffectCacheInternalError({
          ...
          Effect.timeoutFail({
            ...
            onTimeout: () => new EffectCacheTimeoutError({
              message: "list operation timed out",
              timeoutMs: config.timeout,
              operation: "list"
            })
          }),
          Effect.withSpan("EffectCache.list")
        ),
...
export const EffectCacheLive = Layer.scoped(
  ...
  Effect.gen(function* () {
    ...
    const client = yield* Effect.acquireRelease(
      Effect.tryPromise(() => Effect.CacheSDK.connect(config)),
...
list: (params) =>
  Effect.tryPromise({
    ...
    Effect.tap(() => Effect.logDebug("[EffectCache] list called", params)),
    Effect.timeoutFail({ /* ... */ }),
    Effect.withSpan("EffectCache.list")
  ),
```
...
Replace in-memory store in `makeEffectCacheLayer`:
