      ...
      Effect.gen(function* () {
        ...
        if (!item) {
          return yield* Effect.fail(
            new EffectQueueInternalError({ message: `Item ${id} not found` }),
          );
        }
        return item;
      }),
  ...
  Effect.sync(() => {
    ...
    return {
      ...
      healthCheck: Effect.succeed({ status: "healthy" as const }).pipe(
        Effect.withSpan("EffectQueue.healthCheck"),
      ),
 ...
 */
export const EffectQueueTest = Layer.sync(EffectQueue, () => {
  ...
  Effect.gen(function* () {
    ...
    return {
      ...
      list: (params) =>
        store
          .list(params)
          .pipe(Effect.tap(() => Effect.logDebug("[EffectQueue] list called")))
          .pipe(Effect.withSpan("EffectQueue.list")),
      get: (id) =>
        store
          .get(id)
          .pipe(Effect.tap(() => Effect.logDebug(`[EffectQueue] get called: ${id}`)))
          .pipe(Effect.withSpan("EffectQueue.get")),
      create: (data) =>
        store
          .create(data)
          .pipe(Effect.tap(() => Effect.logDebug("[EffectQueue] create called")))
          .pipe(Effect.withSpan("EffectQueue.create")),
      update: (id, data) =>
        store
          .update(id, data)
          .pipe(Effect.tap(() => Effect.logDebug(`[EffectQueue] update called: ${id}`)))
          .pipe(Effect.withSpan("EffectQueue.update")),
      delete: (id) =>
        store
          .delete(id)
          .pipe(Effect.tap(() => Effect.logDebug(`[EffectQueue] delete called: ${id}`)))
          .pipe(Effect.withSpan("EffectQueue.delete")),
    };
  }),
);
