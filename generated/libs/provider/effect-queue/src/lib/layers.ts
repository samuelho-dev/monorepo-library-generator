import { EffectQueueInternalError } from "./errors"
import { EffectQueue } from "./service"
import { env } from "@myorg/env"
import { Effect, Layer, Redacted } from "effect"
import type { EffectQueueConfig, Resource } from "./types"

/**
 * effect-queue - Layer Implementations
 *
  ...
  return {
    ...
    get: (id: string) =>
      Effect.gen(function* () {
        ...
        if (!item) {
          return yield* Effect.fail(new EffectQueueInternalError({ message: `Item ${id} not found` }));
        }
        return item;
      }),
...
//     );
//
//     return EffectQueue.make(client, config);
//   })
// );
//
// // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // 
...
export const EffectQueueLive = Layer.effect(
  ...
  Effect.sync(() => {
    const config: EffectQueueConfig = {
      apiKey: Redacted.value(env.EFFECT_QUEUE_API_KEY) ?? "baseline_api_key",
    ...
    return {
      config,
      healthCheck: Effect.succeed({ status: "healthy" as const }).pipe(Effect.withSpan("EffectQueue.healthCheck")),
      // TODO: Replace store operations with SDK calls
      list: (params) => store.list(params).pipe(Effect.withSpan("EffectQueue.list")),
      get: (id) => store.get(id).pipe(Effect.withSpan("EffectQueue.get")),
      create: (data) => store.create(data).pipe(Effect.withSpan("EffectQueue.create")),
      update: (id, data) => store.update(id, data).pipe(Effect.withSpan("EffectQueue.update")),
      delete: (id) => store.delete(id).pipe(Effect.withSpan("EffectQueue.delete"))
    }
  })
)
...
//   Effect.gen(function* () {
//     const config: EffectQueueConfig = {
//       apiKey: env.EFFECT_QUEUE_API_KEY ?? "baseline_api_key",
//       timeout: env.EFFECT_QUEUE_TIMEOUT ?? 20000,
//     };
//
...
//         // client.disconnect()
//         // pool.end()
//         console.log(`[EffectQueue] Cleaning up client resources`);
//       }),
//     );
//
 ...
 */
export const EffectQueueTest = Layer.sync(
  EffectQueue,
  () => {
 ...
 */
export const EffectQueueDev = Layer.effect(
  EffectQueue,
  Effect.gen(function* () {
    const config: EffectQueueConfig = {
      apiKey: Redacted.value(env.EFFECT_QUEUE_API_KEY) ?? "dev_key",
    ...
    yield* Effect.logInfo("[EffectQueue] [DEV] Development layer initialized with in-memory store");
    ...
    return {
      ...
      healthCheck: Effect.succeed({ status: "healthy" as const }).pipe(Effect.withSpan("EffectQueue.healthCheck")),
      // TODO: Replace store operations with SDK calls
      list: (params) => store.list(params).pipe(Effect.tap(() => Effect.logDebug("[EffectQueue] list called"))).pipe(Effect.withSpan("EffectQueue.list")),
      get: (id) => store.get(id).pipe(Effect.tap(() => Effect.logDebug(`[EffectQueue] get called: ${id}`))).pipe(Effect.withSpan("EffectQueue.get")),
      create: (data) => store.create(data).pipe(Effect.tap(() => Effect.logDebug("[EffectQueue] create called"))).pipe(Effect.withSpan("EffectQueue.create")),
      update: (id, data) => store.update(id, data).pipe(Effect.tap(() => Effect.logDebug(`[EffectQueue] update called: ${id}`))).pipe(Effect.withSpan("EffectQueue.update")),
      delete: (id) => store.delete(id).pipe(Effect.tap(() => Effect.logDebug(`[EffectQueue] delete called: ${id}`))).pipe(Effect.withSpan("EffectQueue.delete")),
    };
  }),
);
 ...
 */
export const EffectQueueAuto = Layer.suspend(() => {
  switch (env.NODE_ENV) {
    case "production":
      return EffectQueueLive;
    case "development":
      return EffectQueueDev;
    case "test":
      return EffectQueueTest;
    default:
      return EffectQueueDev;
  }
});
...
/**
 * makeEffectQueueLayer - Custom layer factory
 *
 ...
 * Example:
 * ```typescript
 * const customLayer = makeEffectQueueLayer({
 *   apiKey: "custom_key",
 *   timeout: 5000,
 * });
 * ```
...
export function makeEffectQueueLayer(config: EffectQueueConfig) {
  return Layer.scoped(
    ...
    Effect.gen(function* () {
      ...
      // Register cleanup function
      yield* Effect.addFinalizer(() =>
        Effect.logInfo("[EffectQueue] [CUSTOM] Cleaning up resources").pipe(
          Effect.andThen(
            Effect.sync(() => {
