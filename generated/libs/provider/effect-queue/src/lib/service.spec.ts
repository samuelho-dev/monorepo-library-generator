 ...
 *
 * @module @myorg/provider-effect-queue
 */
 ...
 */
class EffectQueueTestService extends Context.Tag("EffectQueueTestService")<
  EffectQueueTestService,
  {
 ...
 */
function createEffectQueueTestLayer(config: Record<string, unknown> = {}) {
  return Layer.succeed(EffectQueueTestService, {
    getName: () => Effect.succeed("effect-queue"),
    getConfig: () => Effect.succeed(config),
  });
}

describe("EffectQueue Service", () => {
  describe("Service Interface", () => {
    it.scoped("should provide service through layer", () =>
      Effect.gen(function* () {
        const service = yield* EffectQueueTestService;
        const name = yield* service.getName();
        expect(name).toBe("effect-queue");
      }).pipe(Effect.provide(Layer.fresh(createEffectQueueTestLayer()))),
    );
    ...
    it.scoped("should provide configuration", () =>
      ...
      }).pipe(Effect.provide(Layer.fresh(createEffectQueueTestLayer({ timeout: 5000 })))),
    );
  });
  ...
  describe("Layer Composition", () => {
    ...
    it.scoped("should allow layer override", () => {
      const overrideLayer = Layer.succeed(EffectQueueTestService, {
  ...
  describe("Layer Types", () => {
    it.scoped("should work with Layer.succeed for synchronous initialization", () => {
      const syncLayer = Layer.succeed(EffectQueueTestService, {
        getName: () => Effect.succeed("sync-effect-queue"),
      ...
      return Effect.gen(function* () {
        const service = yield* EffectQueueTestService;
        const name = yield* service.getName();
        expect(name).toBe("sync-effect-queue");
    ...
    it.scoped("should work with Layer.effect for async initialization", () => {
      const asyncLayer = Layer.effect(
        EffectQueueTestService,
        Effect.sync(() => ({
          getName: () => Effect.succeed("async-effect-queue"),
      ...
      return Effect.gen(function* () {
        const service = yield* EffectQueueTestService;
        const name = yield* service.getName();
        expect(name).toBe("async-effect-queue");
    ...
    it.scoped("should work with Layer.scoped for resource management", () => {
      ...
      const scopedLayer = Layer.scoped(
        ...
        Effect.acquireRelease(
          Effect.sync(() => {
            initialized = true;
            return {
              getName: () => Effect.succeed("scoped-effect-queue"),
      ...
      return Effect.gen(function* () {
        const service = yield* EffectQueueTestService;
        const name = yield* service.getName();
        expect(name).toBe("scoped-effect-queue");
