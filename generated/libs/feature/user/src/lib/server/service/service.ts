...
const createServiceImpl = (repo: Effect.Effect.Success<typeof UserRepository>) =>
  ({
    ...
    findByCriteria: (criteria: Record<string, unknown>, offset: number, limit: number) =>
      repo.findAll(criteria as Parameters<typeof repo.findAll>[0], { skip: offset, limit }).pipe(
