import { CacheInternalError, CacheNotFoundError, CacheValidationError } from "./errors";
 ...
 *
 * @module @custom-repo/infra-cache/service
...
export class CacheService extends Context.Tag(
  "@custom-repo/infra-cache/CacheService"
)<
  CacheService,
  {
    /**
    ...
    readonly findByCriteria: (
      ...
      skip?: number,
      limit?: number
    ) => Effect.Effect<readonly unknown[], CacheError>;
    ...
    ) => Effect.Effect<unknown, CacheError>;
     ...
     * ```
     */
    readonly delete: (id: string) => Effect.Effect<void, CacheError>;
...
>() {
   ...
   *
   * This is the live implementation of CacheService that will be used in production.
   ...
   * - Configuration: const config = yield* CacheConfig;
    ...
    Effect.gen(function* () {
      ...
      const resource = yield* Effect.acquireRelease(
        Effect.gen(function* () {
          ...
          //   catch: (error) => new CacheInitializationError({
        ...
        get: (id: string) =>
          Effect.gen(function* () {
            ...
            const result = yield* Effect.tryPromise({
              ...
              catch: (error) => new CacheInternalError({
   ...
   *   Effect.provide(CacheService.Test)
   * );
   * ```
   */
