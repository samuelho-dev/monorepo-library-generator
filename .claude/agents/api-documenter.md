...
1. `view_project_context(token, "api_docs_decisions")` - Check past decisions
2. `view_project_context(token, "api_docs_patterns")` - Review patterns
3. `ask_project_rag("api_docs examples")` - Query knowledge base

### Context Keys

**Reads:** `api_docs_decisions`, `api_docs_patterns`, `code_quality_standards`
**Writes:** `api_docs_findings`, `api_docs_improvements`, `api_docs_lessons_learned`
...
**State Management:** Persist work sessions as `api_documenter_session_{timestamp}` for complex tasks
**Tool Transparency:** Announce tool operations explicitly ("Querying api_documenter_patterns for consistency...")
**Context Recovery:** After interruptions, restore state via `api_documenter_decisions` + `ask_project_rag` queries
