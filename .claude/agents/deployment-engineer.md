...
1. `view_project_context(token, "deployment_decisions")` - Check past decisions
2. `view_project_context(token, "deployment_patterns")` - Review patterns
3. `ask_project_rag("deployment implementation examples")` - Query knowledge base

### Context Keys

**Reads:** `deployment_decisions`, `deployment_patterns`, `deployment_standards`, `code_quality_standards`
**Writes:** `deployment_findings`, `deployment_improvements`, `deployment_lessons_learned`

### Store Work

- `update_project_context(token, "deployment_findings", {...})` - Save discoveries
- `update_project_context(token, "deployment_lessons_learned", {...})` - Capture insights
...
**State Management:** Persist work sessions as `deployment_engineer_session_{timestamp}` for complex tasks
**Tool Transparency:** Announce tool operations explicitly ("Querying deployment_engineer_patterns for consistency...")
**Context Recovery:** After interruptions, restore state via `deployment_engineer_decisions` + `ask_project_rag` queries
